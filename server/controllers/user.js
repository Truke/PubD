...
const login = function *(next) {
  ...
  yield user.query(opts).then(function (doc) {
    ...
    if (user) {
      //签发token
      const token = jwt.sign(JSON.stringify({ userId: user._id, original_iat: Date.now()}), 'PubD');
      ctx.body = {
        msg: '登录成功',
        data: {
          _id : user._id,
...
const register = function *(next) {
  ...
  if(fileNames.length > 0){
    opts.userImg = 'http://127.0.0.1:3030/images/user/' + fileNames[0];
  }
  yield user.query({userName: opts.userName}).then(function (doc) {
    ...
    if (doc) {
      const token = jwt.sign(JSON.stringify({ userId: doc._id, original_iat: Date.now()}), 'PubD');
      ctx.body = {
        msg: '注册成功',
        data: {
          _id : user._id,
...
const userVerify = function *(next) {
  ...
  var profile = jwt.verify(token, 'PubD');
  yield user.query({_id: profile.userId}).then(function (doc) {
    ctx.body = {
      ...
      data: {
        ...
        userNickName : doc[0].userNickName,
        userImg : doc[0].userImg,
      },
      state: true
    }
  })
